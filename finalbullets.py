...
def menu_help():
    ...
    print("'STAT' to generate statistics for an author.")
    print("Q to quit.")
...
def get_stats():
    select = input("Statistics for which author?")
    for author in all_authors:
        if author.keybinding == select.lower():
            logging.info("Getting statistics for {}".format(author.name))
            print("{} selected.".format(author.name))
            print("author # {}".format(author.my_number))
            print("{} total bullets".format(len(author.all_bullets)))
            print("{0} bullets have been sorted by rating {1}%".format(len(author.sorted_bullets), ((len(author.sorted_bullets) / len(author.all_bullets)) * 100)))
            print("{0} bullets shown so far 'show only once {1}%'".format(len(author.bullets_returned_so_far), ((len(author.bullets_returned_so_far) / len(author.all_bullets)) * 100)))
            print("# Rated")
            print("{} Rated: 0".format(sum([bullet.count(0) for bullet in author.all_bullets])))
            print("{} Rated: 1".format(sum([bullet.count(1) for bullet in author.all_bullets])))
            print("{} Rated: 2".format(sum([bullet.count(2) for bullet in author.all_bullets])))
            print("{} Rated: 3".format(sum([bullet.count(3) for bullet in author.all_bullets])))
            print("{} Rated: 4".format(sum([bullet.count(4) for bullet in author.all_bullets])))
            print("{} Rated: 5".format(sum([bullet.count(5) for bullet in author.all_bullets])))
